...
def Rating(result_distance, way, avgGoodGen):
    ...
    for k in range(0, countPopulation):
        for col in range(0, len(result_distance)):
            ...
            if countPopulation - k == countPopulation:
                rating[result_distance[col][k][3] - 1] = [result_distance[col][k][3], (countPopulation * len(way)) + rating[result_distance[col][k][3] - 1][1]]
            else:
