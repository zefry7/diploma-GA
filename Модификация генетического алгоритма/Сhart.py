...
def createChartWithWeather(name, goodGen, avgGoodGen, factGen, avgBestThreeWay):
    fig, axs = plt.subplots(nrows=4, ncols=3)  # Создаем фигуру (необязательно, но позволяет задать размер)
    listStep = []
    listFact = [[[], [], [], [], [], [], [], [], []],
                [[], [], [], [], [], [], [], [], []],
                [[], [], [], [], [], [], [], [], []]]
    ...
    for i in range(countStep):
        listStep.append(i + 1)
        for j in range(len(factGen)):
            for k in range(len(factGen[j])):
                listFact[j][k].append(factGen[j][k])

    for col in range(3):
        for row in range(3):
            axs[row][col].plot(listStep, goodGen[col][row], label='Значение лучшей особи на трассе', color='blue')
            axs[row][col].plot(listStep, avgGoodGen[col][row], label='Среднее значение 10 лучших особей на трассе', color='red')
            axs[row][col].plot(listStep, listFact[col][row], label='Фиксированная особь', color='green')

        axs[3][col].plot(listStep, avgBestThreeWay[col], label='Лучшее среднее значение по трём трассам среди особей', color='red')

    axs[0][0].set_title("Солнечная погода")
    axs[0][1].set_title("Облачная погода")
    axs[0][2].set_title("Пасмурная погода")
